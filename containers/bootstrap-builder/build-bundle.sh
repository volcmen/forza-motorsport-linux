...
if [ "${1:-}" = --fixture ]; then
    ...
    test -d /deps/vendor
    test "$(git -c safe.directory=/src/xodus -C /src/xodus rev-parse --verify 'HEAD^{commit}')" = "$xodus_revision"
    test "$(git -c safe.directory=/src/xgameruntime -C /src/xgameruntime rev-parse --verify 'HEAD^{commit}')" = "$xgameruntime_revision"
    ...
    XDG_CACHE_HOME=/tmp/cache UV_CACHE_DIR=/tmp/uv-cache \
        ...
        --xml /opt/vulkan-registry/vk.xml --video-xml /opt/vulkan-registry/video.xml
